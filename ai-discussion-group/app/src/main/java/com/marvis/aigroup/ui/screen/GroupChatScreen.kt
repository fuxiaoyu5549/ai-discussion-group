...
@Composable
fun GroupChatScreen(convId: String, onBack: () -> Unit) {
    ...
    val roles = remember(conv) { conv.participantRoleIds.mapNotNull { app.getRole(it) } }
    val host = app.getRole(conv.hostRoleId)
    ...
    LaunchedEffect(messages.size, lastMsgId) {
            ...
            TopAppBar(
                title = {
                    Column {
                        ...
                        Text(
                            "${roles.size}个AI参与" + (host?.let { " · 主讲:${it.name}" } ?: ""),
                            fontSize = 11.sp,
        ...
        Column(
            ...
            Row(
                ...
                IconButton(
                    onClick = {
                        val q = input.trim()
                        if (q.isEmpty() || running || roles.isEmpty()) return@IconButton
                        input = ""
                        running = true
                        val roundLimit = 3
                        engine.startGroupDiscussion(conv, q, roles, roundLimit)
                    },
